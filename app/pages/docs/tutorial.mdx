...
votre installation. S'il ne l'est pas, vous devriez voir une erreur
...
`Question` comprend la question sous forme de texte ainsi qu'une liste de choix de
réponse. Un `Choice` contient le choix sous forme de texte, un nombre de votes et la
...
modifier le schéma zod `CreateChoice` afin qu'il inclue les champs requis.
