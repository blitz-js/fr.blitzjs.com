...
l'utiliser afin de générer deux modèles : `Question` et `Choix`. Une
`Question` consiste du texte de la question ainsi qu'une liste de choix de
réponse. Un `Choix` contient le texte du choice, un nombre de votes et la
...
Lorsque suivie par `all`, la commande `generate` génère un modèle ainsi
que toutes ses requêtes, mutations et pages. Pour une liste d'options
complète, visitez [Blitz generate](./cli-generate).
