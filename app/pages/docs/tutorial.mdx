...
Dans ce tutoriel, nous allons vous apprendre à créer use simple
application de vote.
...
Blitz s'occupera de créer un dossier `mon-appli-blitz` dans le répertoire.
Il vous sera également demandé de choisir un module de gestion de
formulaire. Pour les exemples suivant, choisissez le module recommandé
`React Final Form`.

Jettons un oeil à ce que `blitz new` a créé :
...
- Le répertoire `public/` contient toutes vos resources statiques. Si
  votre application nécéssite des images, documents ou vidéos, c'est ici
...
Assurez-vous d'être dans le répertoire `mon-appli-blitz` avant d'exécuter
la commande suivante :
...
Blitz vous permet de générer le code nécéssaire pour une multitude de
...
réponse. Un `Choice` contient le texte du choice, un nombre de votes et la
question à laquelle il fait référence. Blitz s'occupe de générer un `id`
unique, la date de création ainsi que la date de la dernière modification
pour chacun des modèle.
...
`prisma migrate` pour mettre votre schema de base de données à jour avec
...
✔ Enter a name for the new migration: … ajout question
The following migration(s) have been created and applied from new schema changes:

migrations/
  └─ 20210722070215_ajout_question/
    └─ migration.sql
...
pas encore ajouté le champ `Choix` au modèle `Question`. Nous allons donc
choisir `false` lorsqu'on nous demandera de démarrer une migration :
...
#### Finalement, ajoutez à `Question` un relation vers `Choice`.

Ouvrez `db/schema.prisma` et ajoutez `choix Choice[]` au modèle
`Question`.
...
✔ Name of migration … ajout choice
The following migration(s) have been created and applied from new schema changes:

migrations/
  └─ 20210412175528_ajout_choice/
    └─ migration.sql
...
Prisma fournit par Blitz. Pour débuter, exécutez cette commande :
...
⚡ > let q = await db.question.create({data: {texte: "Quoi de neuf?"}})
undefined
...
⚡ > q = await db.question.update({where: {id: 1}, data: {texte: "Ça va?"}})
{
  id: 1,
  createdAt: 2020-06-15T15:06:14.959Z,
...
changements ne seront pas nécéssaires mais pour l'instant ils sont requis
afin de régler quelques bogues.

</Card>

Le contenu des pages générées ne prend pas en compte les attributs définis
lors de la création des modèles. Ce sera le cas bientôt, mais en
...
composant `QuestionsList` à été généré dans une étape précédente :
...
schema zod afin qu'il accepte ces nouveaux choix. Nous devons également
...
Ensuite nous allons créer un fichier séparé où nous allons stocker la
schema de validation zod pour `QuestionForm`. Dans le répertoire
...
l'utiliser dans le schema `QuestionForm` de la prochaine étape.
...
des valeurs par défaut `{{texte: "", choix: []}}` pour nos champs de
`QuestionForm`
...
question au même endroit? Premièrement vous devons personnaliser les
...
Vérifiez le tout en visitant `/questions` avec votre navigateur.
**Magie!**
    ...
    <>
      ...
      <div>
        ...
        <button
          type="button"
          onClick={async () => {
            if (window.confirm("Cette question sera supprimée")) {
...
Cette partie est donc déjà faite pour nous, il ne reste qu'a modifier
notre mutation.
...
## Faites le ménage {#cleanup}

Afin que la commande `yarn tsc` ou `git push` s'exécute sans erreur, vous
devez supprimer `app/choices/mutations/createChoice.ts` (non utilisé) ou
