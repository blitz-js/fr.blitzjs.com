...
Pour les exemples suivant, choisissez le module recommandé
`React Final Form`.
...
schema de validation zod de `CreateQuestion` afin qu'il utilise `text`
plutôt que `name`.
...
+     choice: z.array(z.object({texte: z.string()})),
    })
    ...
    <>
      ...
      <div>
        ...
        <ul>
          {question.choices.map((choice) => (
            <li key={choice.id}>
              {choice.texte} - {choice.votes} votes
